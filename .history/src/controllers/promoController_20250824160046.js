...
exports.addPromo = async (req, res) => {
    ...
    const { nama_produk, barcode, harga_normal, harga_promo, image } = req.body;

    if (isNaN(storeId)) {
        return res.status(400).json({ code: 400, message: 'Invalid store ID' });
    }
    if (!nama_produk || !barcode || !harga_normal || !harga_promo) {
        return res.status(400).json({
            code: 400,
            message: 'Nama produk, barcode, harga normal, dan harga promo wajib diisi'
        });
    }
    ...
    try {
        ...
        // === insert/update ke products (pakai barcode) ===
        const resultProduct = await pool.query(
            `INSERT INTO products (nama_produk, barcode, image)
       VALUES ($1, $2, $3)
       ON CONFLICT (barcode) 
       DO UPDATE SET nama_produk = EXCLUDED.nama_produk,
                     image = EXCLUDED.image
       RETURNING *`,
            [nama_produk, barcode, imagePath]
        );
