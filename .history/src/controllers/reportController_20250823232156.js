...
exports.getAttendance = async (req, res) => {
    ...
    try {
        ...
        // Ubah image path ke base64 jika ada
        const data = await Promise.all(result.rows.map(async (row) => {
            if (row.image) {
                try {
                    const fileData = fs.readFileSync(row.image);
                    const ext = row.image.split('.').pop();
                    row.image = `data:image/${ext};base64,${fileData.toString('base64')}`;
                } catch (e) {
                    row.image = null;
                }
            }
