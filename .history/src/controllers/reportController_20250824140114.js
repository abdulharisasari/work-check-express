...
exports.getTodayCheckinCheckout = async (req, res) => {
    ...
    try {
        const query = `
            SELECT status, time_checkin, time_checkout, date
            FROM attendance
            WHERE user_id = $1
            AND date = $2
            AND status IN (1,2)
        ...
        result.rows.forEach(row => {
            if (row.status === 1) checkin = row.time_checkin;
            if (row.status === 2) checkout = row.time_checkout;
        });

        res.json({
            code: 200,
            message: 'Today check-in & check-out retrieved',
            data: { date: today, time_checkin: checkin, time_checkout: checkout }
        });
